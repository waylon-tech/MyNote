...
* 导入模块 `json`
* `json.dumps(变量名, indent=2, sort_keys=True, ensure_ascii=True)` - 将任意对象 json 序列化为字符串
  * `indent` - 缩进
  * `sort_keys` - 是否排序关键字
  * `ensure_ascii` - `True` 时所有字符都显示为 ascii 格式，`False` 时支持字符显示
* `json.dump(变量名, 文件打开对象)` - 将对象 json 序列化后直接写入一个 `file-like Object`
