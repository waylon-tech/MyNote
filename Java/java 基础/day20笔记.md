...
* 3.IO程序书写
	* 使用前，导入IO包中的类
	* 使用时，进行IO异常处理 (IO在硬盘与内存之间操作东西)
	* 使用后，释放资源

###20.02_IO流(FileInputStream)
* int read()一次读取一个字节，文件的结束标记是-1	**读**
* 
...
* A:案例演示
	* FileOutputStream在创建对象时如果没有这个文件会创建一个，有就会清空这个文件，重写写入
	* FileOutputStream的构造方法写出数据 
	* 如何实现数据的追加写入：在构造方法上第二个参数上传true （注意：针对文件级别的了）
* 
...
###20.09_IO流(定义小数组)
* 以上两种方式都不推荐，推荐↓
* read(byte[] b) 返回读到的有效字节个数，无返回-1
* write(byte[] b, int off, int len) 写出有效的字节个数

		
###20.10_IO流(定义小数组的标准格式)
* A:案例演示		**拷贝3（小数组）**
	* 字节流一次读写一个字节数组复制图片和视频，大小一般为1024的整数倍
	* 忘记加arr的后果，返回的是字节的码表值
		FileInputStream fis = new FileInputStream("致青春.mp3");
		FileOutputStream fos = new FileOutputStream("copy.mp3");
		int len;
