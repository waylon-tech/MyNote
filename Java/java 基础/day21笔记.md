...
* 1.字符流是什么
	...
	* 原理：通过项目默认的码表一次读一个字符
* 2.FileReader extends InputStreamReader 	**读**
	* FileReader类的int read()方法可以按照字符大小读取
* 
...
* FileWriter extends OutputStreamWriter类的write()方法可以自动把字符转为字节写出	**写**
* 注意事项：
* a.没有文件时会创建
* b.加true会追加
* c.参数为String、int皆可
...
* 注意事项：
* a.漏写close() 没有结果，因为底层（writer类）建立了缓冲区，容纳1024个字符，大小为2k
...
* InputStreamReader与OutputStreamReader是字节与字符的桥梁，字符流相当于内置了一个编码表
...
* 注意事项
* a.new FileWriter("xxx.txt") 就会先清空文件内容
* b.没有关流，会出现NumberFormatException,原因见上文

###21.16_File类(递归)
* 方法自己调用自己，一个跳出条件，和一个递归方式
* 理解：类似于归纳法，一个初始值，一个关系
* 特点：
* a.循环 很多时候需要能够明确循环次数
* b.递归 简单判断就行
* 弊端：容易导致占内存溢出
