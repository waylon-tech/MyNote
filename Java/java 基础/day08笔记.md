...
* A:什么是方法重写
	* 重写:子父类出现了一模一样的方法(注意:返回值类型可以是子父类,这个我们学完面向对象讲)（子类比父类更强大，是ok的）
* B:方法重写的应用：
...
* A:final修饰变量的初始化时机
	* 显示初始化 final int x = 10; 默认初始化值是无效值（特别珍惜！）
	* 在对象构造完毕前即可 （可分开，在构造方法上赋值）
	
###08.22_day08总结
* 把今天的知识点总结一遍。
* 总结：
* 一、代码块
* 二、继承（相关概念、this与super之于成员变量与方法）
* 三、重写与重载
* 四、final