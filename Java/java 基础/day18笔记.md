...
* A:Map集合的功能概述
	* a:**添加功能**
		* V put(K key,V value):添加元素，返回被覆盖的值
...
* A:面试题
	* HashMap和Hashtable的区别
		...
		* Hashtable不可以存储null键和null值(NullPointerException,为了后续代码可以继续执行),HashMap可以存储null键和null值
* B:案例演示	
	* HashMap和Hashtable的区别
...
* B:Collections成员方法
* 
		public static <T> void sort(List<T> list)	list集合进行排序，类需要具备比较性
		public static <T> int binarySearch(List<?> list,T key)	二分法查找元素的索引。若够有，返回索引，如果无，返回负的插入点减一
		public static <T> T max(Collection<?> coll)	按CompareTo获取集合中的最大值
		public static <T> T min(Collection<?> coll)	按CompareTo获取集合中的最小值
...
###18.15_集合框架(泛型固定下边界)
* ? super E 拿出来（到比较器中作比较）
* ? ectends E 放进去（到集合中）

###18.16_day18总结
* 把今天的知识点总结一遍。
* 一、Map集合（概述、功能、遍历12）
* 二、HashMap、LinkedHashMap、TreeMap集合及类比
* 三、Map嵌套，HashMap和Hashtable区别
* 四、Collections集合工具类
* 五、泛型固定下边界，泛型固定上边界